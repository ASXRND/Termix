...
**Быстрый повторный цикл:** `SKIP_NATIVE=1 npm run build:mac-local` пропускает
патчи `node_modules` и `electron-rebuild` (нативные модули уже собраны под
текущий Electron) — остаётся `vite build` + `electron-builder` (~40 секунд).
Полный прогон нужен после смены версии Electron, `node-pty`, `better-sqlite3`
или на свежем клоне.
