 ...
 * split. Reads the local FS over IPC (electron/local-fs.cjs).
 */
export function LocalFileExplorer({ onClose }: { onClose: () => void }) {
  ...
  // Resolve the tree root once: the parent of the home dir, so the user can
  // still navigate above their home folder like in VS Code.
  useEffect(() => {
    let cancelled = false;
    (async () => {
  ...
  const loadChildren = useCallback(async (rootPath: string, rel: string) => {
    ...
    setTree((prev) => {
      const base =
        prev ??
        { name: "", rel: "", absPath: rootPath, isDir: true, children: [] };
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
      ...
      {root && (
        <div
          className="truncate border-b border-sidebar-border px-3 py-1.5 text-[11px] text-muted-foreground"
          title={root}
        >
          {root}
        </div>
